...
> Open Question: How many pieces compose a full pod?
We have confirmed that there are at least 4,5 components of a full pod. Most of the time, cells will not be addressed directly for I/O handling. The handling of the golem graph data will determine if there are more.
...
- c_ind : the index of the cell at a pod
...
> Open Question: What should be used as the pod between semantically named layers? The order? What about cross-region references?
...
| pod | the address of a list of cells that handles I/O processing for it's cells | [pd_key, r_key, l_key, l_pos] |
