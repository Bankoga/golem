...
class TestCollectorSegment(TestPlasticComp):
  ...
  def set_up_var(self):
    ...
    self.values = [self.fill_shape,self.spacing_shape]
    self.var = ConvVar(fill_shape=self.fill_shape, spacing_shape=self.spacing_shape)
  ...
  def setUp(self):
    ...
    self.comp = CollectorSegment(self.fill_shape, self.spacing_shape,label=self.label)
  ...
  def test_prepare_var_args(self, f_shape,s_shape):
    ...
    expectation = ConvVar(fill_shape=f_shape, spacing_shape=s_shape)
    result = self.comp.prepare_var_args(*var_args)
    self.assertEqual(result, expectation)
    self.assertEqual(result.fill_shape, f_shape)
    self.assertEqual(result.spacing_shape, s_shape)
    self.assertEqual(self.comp.shape, result.fill_shape)
  ...
  def test_update(self,f_shape,s_shape):
    ...
    self.assertEqual(self.comp.var, ConvVar(fill_shape=f_shape,spacing_shape=s_shape))
    self.assertEqual(self.comp.weights.shape, self.comp.fill_shape)
